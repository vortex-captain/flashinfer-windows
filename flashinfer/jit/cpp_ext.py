...
def build_cuda_cflags(
    ...
    if is_windows:
        cuda_cflags += ["$common_cuda_flags", "--expt-relaxed-constexpr"]
    else:
        cuda_cflags += [
            "$common_cflags",
...
def generate_ninja_build_for_op(
    ...
    if is_windows and needs_device_linking:
        ldflags = [
            f'-L"{flag[len("/LIBPATH:"):]}"' if flag.startswith("/LIBPATH:") else flag
            for flag in ldflags
        ]
    ...
    if embedded_cubins:
        ...
        for index, (cubin_name, cubin_path) in enumerate(
            ...
            cubin_dependency = str(resolved_cubin)
            if is_windows:
                embedded_obj = embedded_obj.replace(":\\", "$:\\")
                cubin_dependency = cubin_dependency.replace(":\\", "$:\\")
            lines.extend(
                [
                    f"build {embedded_obj}: embed_cubin {current_obj} | {cubin_dependency}",
