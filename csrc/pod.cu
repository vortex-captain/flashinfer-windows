...
void pod_with_kv_cache_tensor_impl_(
    PrefillParams& prefill_params, DecodeParams& decode_params, const int64_t* k_strides_d,
    const int64_t* v_strides_d,
  ...
  using IdType = typename DecodeParams::IdType;
  ...
  }
  {
    ...
    paged_kv_t<DTypeKV, IdType> paged_kv(
		...
		static_cast<DTypeKV*>(paged_v_cache_d.data_ptr()), k_strides_d, v_strides_d,
    ...
    params.maybe_mask_indptr = maybe_mask_indptr_d.has_value()
								 ? static_cast<IdType*>(maybe_mask_indptr_d.value().data_ptr())
								 : nullptr;
  ...
  DISPATCH_context(
      ...
      USE_SLIDING_WINDOW_D, USE_LOGITS_SOFT_CAP, [&] {
        ...
        pod_with_kv_cache_tensor_impl_<MASK_MODE_P, MASK_MODE_D, DTypeQ, DTypeO, DTypeKV,
            ...
            prefill_params, decode_params, k_strides_d.data(), v_strides_d.data(),
            q_p, k_p, v_p, o_p, maybe_lse_p,
